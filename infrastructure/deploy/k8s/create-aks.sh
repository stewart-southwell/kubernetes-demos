...
demoRegistry=${DEMO_REGISTRY}
demoAcrName=${DEMO_ACRNAME}
clusterClientId=${DEMO_CLIENTID}
clusterClientSecret=${DEMO_CLIENTSECRET}
...
then
    echo
    echo "Granting AKS pull permissions from ACR $demoAcrName"
    az aks update -n $clusterAksName -g $clusterRg --attach-acr $demoAcrName
fi
...
if [ ! -z "$demoAcrName" ]
then
    echo export DEMO_ACRNAME=$demoAcrName >> create-aks-exports.txt
fi

if [ ! -z "$demoRegistry" ]
then
    echo export DEMO_REGISTRY=$demoRegistry >> create-aks-exports.txt
fi

if [ ! -z "$spHomepage" ]
then
   echo export DEMO_CLIENTID=$clusterClientId >> create-aks-exports.txt
   echo export DEMOCLIENTPASSWORD=$clusterClientSecret >> create-aks-exports.txt
fi

if [ -z "$DEMO_QUICKSTART" ]
then
    echo "Run the following command to update the environment"
    echo 'eval $(cat ~/clouddrive/aks-demo/create-aks-exports.txt)'
    echo
fi

mv -f create-aks-exports.txt ~/clouddrive/aks-demo/